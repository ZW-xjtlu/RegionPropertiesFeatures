...
sequenceDerivedFeatures <- function(x,
  ...
  seqfeatures <- assign_method(sequence_M, encoding)
    ...
    overlap <- suppressWarnings(findOverlaps(mutation, x, ignore.strand = TRUE))
    ...
    if(length(QrHit) == 0){
      stop("No mutations can be mapped to x.")
        ...
        sequences_mutated[!indx_minus_range][pos_plus_indx] <- DNAStringSet( nt_replace_plus[!indx_minus_range] )
        pos_minus_indx <- split(pos_replace_minus,seq_along(pos_replace_minus))[indx_minus_range]
        names(pos_minus_indx) <- NULL
        sequences_mutated[indx_minus_range][pos_minus_indx] <- DNAStringSet( nt_replace_minus[indx_minus_range] )
        rm(pos_replace_plus, pos_replace_minus, indx_minus_range, pos_plus_indx, pos_minus_indx)
        sequence_M_ref <- matrix( unlist( strsplit(as.character(sequences_reference),"") ), ncol =  N, byrow = TRUE)
        seqfeatures_ref <- assign_method(sequence_M_ref, encoding)
        rm(sequence_M_ref, sequences_reference)
        sequence_M_mut <- matrix( unlist( strsplit(as.character(sequences_mutated),"") ), ncol =  N, byrow = TRUE)
        ...
        feature_lst <- list(ref = seqfeatures_ref,
                            mut = seqfeatures_mut)
        if(cnn) feature_lst <- lapply(feature_lst, function(x) wrap_cnn(x, N))
...
wrap_cnn <- function(seqfeatures, N){
  ...
  cnn_array <- array(as.vector(seqfeatures),dim=c(nrow(seqfeatures),num_features,N))
